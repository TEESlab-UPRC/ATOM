...
##--------------FutureAgent_Net-Metering--------------------------------------------------------------------------------------------##
######################################################################################################################################


class FutureAgent_NetMetering(Agent):
    ...
    def inform(self, revenue, option):
        ...
        self.expectations[option.name][1] = indicator_3(np.random.uniform(0.95, 1.)*belief_var)
        self.expectations[option.name][0] = 0.5*belief_mean + revenue*np.random.uniform(0.2,0.5)
    ...
    def update_revenue(self):
        self.revenue = self.model.calculate_revenue(self.my_option, self.model.initial_demand, self.model.demand_change,
                                        self.model.demand_dfact, self.model.init_ret_price1, self.model.init_ret_price2,
                                        self.model.ret_price_change, self.model.ret_price_change_rate, self.model.start_year, self.model.end_year)
